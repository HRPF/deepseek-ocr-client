...
def update_progress(
    ...
    with progress_lock:
        ...
        if chars_generated > 0:
            ...
            logger.info(
                f"Progress: {status} - {stage} - {message} ({progress_percent}%)"
            )
...
def load_model_background():
    ...
    try:
        ...
        is_cached = (
            initial_cache_size > 100 * 1024 * 1024
        )  # More than 100 MB suggests model is cached
        ...
        if is_cached:
            ...
            update_progress(
                "loading",
                "model",
                "Downloading model files (this will take several minutes)...",
                25,
            )
        ...
        download_monitor_active = [
            True
        ]  # Use list for mutable access in nested function
            ...
            while download_monitor_active[0] and progress < 75:
                ...
                if current_size > last_size:
                    ...
                    update_progress(
                        "loading",
                        "model",
                        f"Downloading model files... ({size_mb:.1f} MB downloaded)",
                        progress,
                    )
        ...
        try:
            ...
            logger.warning(
                f"Flash attention not available: {e}, using default attention"
            )
            model = AutoModel.from_pretrained(
                MODEL_NAME,
                trust_remote_code=True,
...
def perform_ocr():
    ...
    try:
        ...
        prompt_configs = {
            ...
            "free": {"prompt": "<image>\nFree OCR. ", "output_file": "result.txt"},
            "figure": {
                "prompt": "<image>\nParse the figure. ",
                "output_file": "result.txt",
            },
        ...
        class CharCountingStream:
            ...
            def write(self, text):
                ...
                try:
                    ...
                    try:
                        safe_text = text.encode(
                            self.original.encoding or "utf-8", errors="replace"
                ...
                self.section_count = self.accumulated_text.count(
                    "=" * 20
                )  # Count long === lines
        ...
        if os.path.exists(result_filepath):
            ...
            logger.warning(
                f"Expected file '{expected_output_file}' not found, searching for alternatives"
            )
        ...
        return jsonify(
            {
                "status": "success",
                "result": result_text,
                "boxes_image_path": "result_with_boxes.jpg"
                if has_boxes_image
                else None,
                "prompt_type": prompt_type,
